# Cu PVD hybrid Zapdos input template — Dirichlet-target 2D axisymmetric R-Z geometry.
# Prescribed-source startup plus resolved relative wafer RF bias:
#   - 30 cm radius/height chamber surrogate
#   - Cu-only chemistry
#   - pre-filled quasi-neutral magnetron plasma
#   - magnetic electron transport enabled
#   - wafer/bottom chuck RF bias imposed as a resolved Dirichlet electrode
#     with a gentle startup ramp
#   - target metal voltage is imposed directly as a Dirichlet potential
#   - prescribed nonlocal fast-electron Cu ionization/energy source tables ramp on
#     for startup and then decay to a smaller support level
#   - target secondary electron emission remains enabled
#   - mesh is strongly refined near the wafer so the resolved RF sheath is not
#     forced into one coarse cell
#
# Coordinates:
...
#
# Physics convention:
#   The target is kept as a metal Dirichlet boundary. Because that pulls the
#   solved potential reference negative, the wafer metal voltage is specified
#   relative to the computed/estimated wafer-side plasma potential rather than
#   relative to lab ground:
#
#     V_wafer,dc = phi_wafer_plasma_reference - desired_wafer_sheath_voltage
#
#   In this resolved-wafer version the wafer boundary is the imposed metal
#   voltage, so there is no unresolved wafer-sheath diagnostic. Estimate any
#   wafer-side plasma reference from the near-wafer potential field in Exodus.
#
...
#     -i zapdos_templates/cu_pvd_hybrid_dirichlet_target_relative_wafer_rf_30cm.i
#
...
wafer_radius       = 0.15
target_voltage     = -300
wafer_plasma_potential_reference = -280
desired_wafer_sheath_voltage = 30
wafer_rf_voltage   = 30
wafer_rf_frequency = 13.56e6
wafer_rf_ramp_time = 3.0e-7
# Keep target SEE active while the prescribed effective source is reduced.
# The prescribed source represents a coarse nonlocal seed/support term; SEE is
# still transport-coupled through the target ion flux.
gamma_secondary            = 0.15
...
# They ramp on for startup, then decay from initial_scale to final_scale so the
# prescribed map does not permanently pin the plasma expansion.
effective_source_initial_scale = 0.2
effective_source_final_scale   = 0.001
effective_source_ramp_time     = 1.0e-8
effective_source_decay_start   = 1.0e-7
effective_source_decay_time    = 1.0e-7
...
# With the wafer potential imposed as a Dirichlet boundary, the potential drop
# is solved in the mesh. These reflection/loss controls only damp wall
# collection during startup; they are not a separate sheath diagnostic model.
wafer_electron_reflection = 0.5
bottom_shield_electron_reflection = 0.5
right_wall_electron_reflection = 0.4

wafer_ion_loss_scale = 0.1
bottom_shield_ion_loss_scale = 0.1
...
[Mesh]
  ...
  [generated]
    ...
    # First wafer-adjacent cell is about 0.078 mm for this 30 cm domain.
    bias_y = 1.02
    elem_type = QUAD4
  []
...
[AuxKernels]
  ...
  [S_Cu_eff_aux]
    type = FunctionAux
    variable = S_Cu_eff
    function = effective_cu_ionization_source_applied_density
  ...
  [Qe_eff_aux]
    type = FunctionAux
    variable = Qe_eff
    function = effective_electron_energy_source_applied_density
    execute_on = 'INITIAL TIMESTEP_BEGIN TIMESTEP_END'
  []
[]
...
[BCs]
  ...
  # Resolved RF bottom electrode. This lets Poisson solve the wafer-side sheath
  # field directly, so the wafer mesh is refined strongly near y=0.
  [wafer_potential]
    type = FunctionDirichletBC
    variable = potential
    boundary = wafer
...
[Functions]
  ...
  [wafer_voltage_func]
    type = ParsedFunction
    expression = '(${wafer_plasma_potential_reference}-${desired_wafer_sheath_voltage}) + tanh(t/${wafer_rf_ramp_time})*${wafer_rf_voltage}*sin(6.283185307179586*${wafer_rf_frequency}*t)'
  []
  ...
  [effective_cu_ionization_source_applied_density]
    type = ParsedFunction
    symbol_names = 'S'
    symbol_values = 'effective_cu_ionization_source_density'
    expression = 'S*tanh(t/${effective_source_ramp_time})*(${effective_source_final_scale} + (${effective_source_initial_scale}-${effective_source_final_scale})*exp(-0.5*(t-${effective_source_decay_start}+sqrt((t-${effective_source_decay_start})^2))/${effective_source_decay_time}))'
  []

  [effective_cu_ionization_source_molar]
    type = ParsedFunction
    # Kernel source for log-molar Zapdos variables. The applied density source
    # ramps on and then decays from initial_scale to final_scale.
    symbol_names = 'S'
    symbol_values = 'effective_cu_ionization_source_applied_density'
    expression = 'S/${avogadro}'
  []
  ...
  [effective_electron_energy_source_applied_density]
    type = ParsedFunction
    symbol_names = 'Q'
    symbol_values = 'effective_electron_energy_source_density'
    expression = 'Q*tanh(t/${effective_source_ramp_time})*(${effective_source_final_scale} + (${effective_source_initial_scale}-${effective_source_final_scale})*exp(-0.5*(t-${effective_source_decay_start}+sqrt((t-${effective_source_decay_start})^2))/${effective_source_decay_time}))'
  []
  ...
  [effective_electron_energy_source_molar]
    ...
    symbol_names = 'Q'
    symbol_values = 'effective_electron_energy_source_applied_density'
    expression = 'Q/${avogadro}'
  []

  [potential_ic]
    type = ParsedFunction
    # Initial guess connects the observed wafer-side plasma reference to the
    # powered target metal potential.
    expression = '${wafer_plasma_potential_reference} + (${target_voltage}-${wafer_plasma_potential_reference})*(y/${chamber_length})'
  []
...
[Executioner]
  ...
  [TimeSteppers]
    [Adaptive]
      ...
      dt                 = 1.0e-12
      growth_factor      = 1.2   # keep the source/sheath startup gradual
      optimal_iterations = 5       # target smaller accepted steps near stiff source growth
    []
  []
[]
