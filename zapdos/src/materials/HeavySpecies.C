...
HeavySpeciesTempl<is_ad>::validParams()
{
  ...
  params.addParam<Real>(
      "reduced_mobility",
      "Reduced mobility at the reference pressure and temperature. If supplied, the species "
      "mobility is computed as reduced_mobility * reference_pressure / heavy_species_p * "
      "heavy_species_T / reference_temperature, and the diffusivity is computed from the "
      "Einstein relation.");
  params.addParam<Real>("reduced_mobility_reference_pressure",
                        101325.0,
                        "Reference pressure for reduced mobility scaling, in Pa.");
  params.addParam<Real>("reduced_mobility_reference_temperature",
                        273.15,
                        "Reference temperature for reduced mobility scaling, in K.");
...
HeavySpeciesTempl<is_ad>::HeavySpeciesTempl(const InputParameters & parameters)
  : Material(parameters),
    ...
    _use_reduced_mobility(isParamValid("reduced_mobility")),
    _reduced_mobility(_use_reduced_mobility ? getParam<Real>("reduced_mobility") : 0.0),
    _reduced_mobility_reference_pressure(getParam<Real>("reduced_mobility_reference_pressure")),
    _reduced_mobility_reference_temperature(getParam<Real>("reduced_mobility_reference_temperature"))

{
  if (_use_reduced_mobility && (isParamValid("mobility") || isParamValid("diffusivity")))
    paramError("reduced_mobility",
               "Do not specify mobility or diffusivity when reduced_mobility is supplied.");

  if (_use_reduced_mobility)
  {
    _calc_mobility = true;
...
HeavySpeciesTempl<is_ad>::computeQpProperties()
{
  ...
  if (_use_reduced_mobility)
  {
    _muHeavy[_qp] = _reduced_mobility * _reduced_mobility_reference_pressure /
                   _pressureHeavy[_qp] * _temperatureHeavy[_qp] /
                   _reduced_mobility_reference_temperature * _voltage_scaling * _time_units;
