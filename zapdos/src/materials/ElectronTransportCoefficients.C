...
ElectronTransportCoefficients::validParams()
{
  ...
  params.addParam<bool>(
      ...
      false,
      "Clamp exp(electron_energy - electrons) before sampling transport coefficient tables.");
  params.addParam<Real>(
      "actual_mean_energy_min",
      0.0,
      "Lower bound in eV for the actual mean electron energy used for transport interpolation. "
      "If non-positive, the first table energy is used when clamping is enabled.");
  params.addParam<Real>(
      "actual_mean_energy_max",
      0.0,
      "Upper bound in eV for the actual mean electron energy used for transport interpolation. "
    ...
    _mean_en(adCoupledValue("electron_energy"))
{
  ...
  {
    if (actual_mean_energy.empty())
      paramError("property_tables_file", "The electron transport property table is empty.");
    if (_actual_mean_energy_min <= 0.0)
      _actual_mean_energy_min = actual_mean_energy.front();
    if (_actual_mean_energy_max <= 0.0)
      _actual_mean_energy_max = actual_mean_energy.back();
    if (_actual_mean_energy_min <= 0.0)
...
ADReal
ElectronTransportCoefficients::actualMeanEnergy() const
{
  ADReal actual_mean_energy;
  const Real log_actual_mean_energy = _mean_en[_qp].value() - _em[_qp].value();
  actual_mean_energy.value() = std::exp(log_actual_mean_energy);
  actual_mean_energy.derivatives() =
      actual_mean_energy.value() * (_mean_en[_qp].derivatives() - _em[_qp].derivatives());
  if (!_clamp_actual_mean_energy)
    return actual_mean_energy;

  const Real value = actual_mean_energy.value();
...
ElectronTransportCoefficients::computeQpProperties()
{
  ...
  const ADReal actual_mean_energy = this->actualMeanEnergy();
  ...
  if (_pressure_dependent) // If the mobility and diff. does depend on pressure
  {
    ...
    if (_interp_trans_coeffs)
    {
      if (_ramp_trans_coeffs)
      {
        _muem[_qp].value() =
            (std::tanh(_t / 1e-6) *
                 _mu_interpolation.sample(actual_mean_energy_value) +
             (1. - std::tanh(_t / 1e-6)) * .0352) *
            _voltage_scaling * _time_units * N_inverse;
        _muem[_qp].derivatives() =
            std::tanh(_t / 1e-6) *
            _mu_interpolation.sampleDerivative(actual_mean_energy_value) *
            actual_mean_energy.derivatives() * _voltage_scaling * _time_units * N_inverse;
        _diffem[_qp].value() =
            (std::tanh(_t / 1e-6) *
                 _diff_interpolation.sample(actual_mean_energy_value) +
             (1. - std::tanh(_t / 1e-6)) * .30) *
            _time_units * N_inverse;
        _diffem[_qp].derivatives() = std::tanh(_t / 1e-6) *
                                     _diff_interpolation.sampleDerivative(actual_mean_energy_value) *
                                     actual_mean_energy.derivatives() *
                                     _time_units * N_inverse;
      }
      else
      {
        ...
        _muem[_qp].derivatives() =
            ...
            actual_mean_energy.derivatives() * _voltage_scaling * _time_units * N_inverse;
        _diffem[_qp].value() =
            _diff_interpolation.sample(actual_mean_energy_value) * _time_units * N_inverse;
        _diffem[_qp].derivatives() =
            _diff_interpolation.sampleDerivative(actual_mean_energy_value) *
            actual_mean_energy.derivatives() * _time_units * N_inverse;
      }
    }
    else
    {
    ...
    if (_interp_trans_coeffs)
    {
      if (_ramp_trans_coeffs)
      {
        ...
        _muem[_qp].derivatives() =
            std::tanh(_t / 1e-6) *
            _mu_interpolation.sampleDerivative(actual_mean_energy_value) *
            actual_mean_energy.derivatives() * _voltage_scaling * _time_units;
        _diffem[_qp].value() =
            std::tanh(_t / 1e-6) *
                _diff_interpolation.sample(actual_mean_energy_value) +
      ...
      else
      {
        ...
        _muem[_qp].derivatives() =
            ...
            actual_mean_energy.derivatives() * _voltage_scaling * _time_units;
        _diffem[_qp].value() =
            _diff_interpolation.sample(actual_mean_energy_value) * _time_units;
        _diffem[_qp].derivatives() =
            _diff_interpolation.sampleDerivative(actual_mean_energy_value) *
            actual_mean_energy.derivatives() * _time_units;
      }
    }
    else
    {
