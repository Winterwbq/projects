...
AddDriftDiffusionAction::validParams()
{
  ...
  params.addParam<std::vector<std::string>>(
      ...
      " EField");
  params.addParam<bool>("use_magnetized_electron_transport",
                        false,
                        "Use prescribed magnetic-field transport kernels for electrons and "
                        "electron mean energy.");
  params.addParam<bool>("use_magnetized_ion_transport",
                        false,
                        "Use prescribed magnetic-field transport kernels for ions.");
  params.addParam<FunctionName>(
      "magnetic_field_r",
      "0",
      "Radial magnetic field Br in Tesla for magnetized transport.");
  params.addParam<FunctionName>(
      "magnetic_field_z",
      "0",
      "Axial magnetic field Bz in Tesla for magnetized transport.");
                                      ...
                                      const bool & energy)
{
  ...
  const bool use_magnetized_transport =
      (getParam<bool>("use_magnetized_electron_transport") &&
       ((isParamValid("electrons") && name == getParam<NonlinearVariableName>("electrons")) ||
        (isParamValid("electron_energy") &&
         name == getParam<NonlinearVariableName>("electron_energy")))) ||
      (getParam<bool>("use_magnetized_ion_transport") && charged &&
       (!isParamValid("electrons") || name != getParam<NonlinearVariableName>("electrons")));
  ...
  if (charged)
  {
    InputParameters params1 = _factory.getValidParams(use_magnetized_transport
                                                          ? "MagnetizedEFieldAdvection"
                                                          : "EFieldAdvection");
    ...
    if (use_magnetized_transport)
    {
      params1.set<FunctionName>("magnetic_field_r") = getParam<FunctionName>("magnetic_field_r");
      params1.set<FunctionName>("magnetic_field_z") = getParam<FunctionName>("magnetic_field_z");
    }
    params1.set<std::vector<SubdomainName>>("block") =
        getParam<std::vector<SubdomainName>>("block");
    _problem->addKernel(use_magnetized_transport ? "MagnetizedEFieldAdvection" : "EFieldAdvection",
                        name + "_advection",
                        params1);
  }

  InputParameters params2 =
      _factory.getValidParams(use_magnetized_transport ? "MagnetizedCoeffDiffusion"
                                                       : "CoeffDiffusion");
  params2.set<NonlinearVariableName>("variable") = {name};
  params2.set<Real>("position_units") = getParam<Real>("position_units");
  if (use_magnetized_transport)
  {
    params2.set<FunctionName>("magnetic_field_r") = getParam<FunctionName>("magnetic_field_r");
    params2.set<FunctionName>("magnetic_field_z") = getParam<FunctionName>("magnetic_field_z");
  }
  params2.set<std::vector<SubdomainName>>("block") = getParam<std::vector<SubdomainName>>("block");
  _problem->addKernel(use_magnetized_transport ? "MagnetizedCoeffDiffusion" : "CoeffDiffusion",
                      name + "_diffusion",
                      params2);

  if (energy)
  {
    InputParameters params3 = _factory.getValidParams(use_magnetized_transport
                                                          ? "MagnetizedEFieldAdvection"
                                                          : "EFieldAdvection");
    ...
    if (use_magnetized_transport)
    {
      params3.set<FunctionName>("magnetic_field_r") = getParam<FunctionName>("magnetic_field_r");
      params3.set<FunctionName>("magnetic_field_z") = getParam<FunctionName>("magnetic_field_z");
    }
    ...
    InputParameters params4 = _factory.getValidParams(use_magnetized_transport
                                                          ? "MagnetizedJouleHeating"
                                                          : "JouleHeating");
    ...
    if (use_magnetized_transport)
    {
      params4.set<FunctionName>("magnetic_field_r") = getParam<FunctionName>("magnetic_field_r");
      params4.set<FunctionName>("magnetic_field_z") = getParam<FunctionName>("magnetic_field_z");
    }
    params4.set<std::vector<SubdomainName>>("block") =
        getParam<std::vector<SubdomainName>>("block");
    _problem->addKernel(use_magnetized_transport ? "MagnetizedJouleHeating" : "JouleHeating",
                        name + "_joule_heating",
                        params4);
  }
