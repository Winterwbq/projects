# Reduced electrostatic Ar ignition test with prescribed-Bz magnetized electron transport
# and an approximate SEE-driven sheath ionization loop.
...
secondary_emission_ramp_time = 5.0e-8
see_source_length = 1.0e-4
see_ionization_efficiency = 0.50
see_max_ionizations_per_secondary = 30.0
...
[BCs]
  ...
  [target_secondary_electron_energy]
    ...
    r = 0
    emission_coeffs = gamma_target
    secondary_electron_energy = ${see_bulk_energy_per_pair}
    position_units = ${position_scale}
  []
...
[Postprocessors]
  ...
  [electron_density_average]
    type = ElementAverageValue
    variable = em_density
    execute_on = 'INITIAL TIMESTEP_END'
  []

  [electron_density_inventory]
    type = ElementIntegralVariablePostprocessor
    variable = em_density
    execute_on = 'INITIAL TIMESTEP_END'
  []

  [electron_log_density_max]
    type = ElementExtremeValue
    variable = em
    execute_on = 'INITIAL TIMESTEP_END'
  []

  [electron_log_density_min]
    type = ElementExtremeValue
    variable = em
    value_type = min
    execute_on = 'INITIAL TIMESTEP_END'
  ...
  [argon_ion_density_average]
    type = ElementAverageValue
    variable = Ar+_density
    execute_on = 'INITIAL TIMESTEP_END'
  []

  [argon_ion_density_inventory]
    type = ElementIntegralVariablePostprocessor
    variable = Ar+_density
    execute_on = 'INITIAL TIMESTEP_END'
  []
